...
export function testRuleset(id) {
        ...
        .onKeyType(() => {
            ...
            for(let i = 0; i < results.length; i++) {
                ...
                let resultContainer = new UIContainer()
                    .setX(new CramSiblingConstraint(8)).setY(new CramSiblingConstraint(8))
                    .setWidth(new SubtractiveConstraint((50).percent(), (15).pixels())).setHeight((10).percent())
                    .setChildOf(scroll);
                ...
                image.setColor(color1)
                    .setX(new SubtractiveConstraint(new CenterConstraint(), (25).percent())).setY(new CenterConstraint())
                    .setWidth((result ? 20 : 16).pixels()).setHeight(new AspectConstraint())
                    .setChildOf(resultContainer);

                new UIText("Rule #"+(i+1))
                    .setColor(color2)
                    .setX((40).percent()).setY(new AdditiveConstraint(new CenterConstraint(), (2).pixels()))
                    .setTextScale((1.5).pixels())
                    .setChildOf(resultContainer);
            }
        })
    ...
    const failIcon = UIImage.ofFile(new File("./config/ChatTriggers/modules/HyJanitor/icons/g-close.png"))
        .setColor(colorScheme.dark.primary.color)
        .setX(new AdditiveConstraint((100).percent(), (8).pixels())).setY(new CenterConstraint())
        .setWidth((28).pixels()).setHeight(new AspectConstraint())
        .setChildOf(inputOutline);
    ...
    const mainRectangle = new UIRoundedRectangle(20)
        ...
        .setX(new CenterConstraint()).setY((40).percent())
        .setWidth((30).percent()).setHeight((50).percent())
        .setChildOf(background);

    const time = new UIText("Evaluated in 0.00 ms")
        .setColor(colorScheme.dark.surfaceVariant.color)
        .setX(new CenterConstraint()).setY(new AdditiveConstraint((100).percent(), (8).pixels()))
        .setChildOf(mainRectangle)

    const scroll = new ScrollComponent()
        .setX(new CenterConstraint()).setY(new CenterConstraint())
        .setWidth(new SubtractiveConstraint((100).percent(), (4).pixels())).setHeight(new SubtractiveConstraint((100).percent(), (4).pixels()))
        .setChildOf(mainRectangle);

    const slider = new UIBlock()
        .setColor(colorScheme.dark.outline.color)
        .setX(new SubtractiveConstraint((100).percent(), (18).pixels())).setY((5).pixels())
